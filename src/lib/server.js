...
server.tool(
    ...
    "sends keys to an element, aka typing",
    {
        ...
        slowly: z.boolean().optional().describe("Whether to type one character at a time with delays. Useful for triggering key handlers in the page. By default entire text is filled in at once.")
    },
    async ({ by, value, text, timeout = 10000, slowly = false }) => {
        try {
            const driver = getDriver();
            const locator = getLocator(by, value);
            const element = await driver.wait(until.elementLocated(locator), timeout);
            ...
            return {
                content: [{ type: 'text', text: `Text "${text}" entered into element${slowly ? ' (slowly)' : ''}` }]
            };
        } catch (e) {
            return {
